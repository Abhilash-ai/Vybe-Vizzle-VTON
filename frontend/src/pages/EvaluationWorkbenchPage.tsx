...
import { ProviderStatusCard } from '../components/evaluation/ProviderStatusCard';
  ...
  return (
    <div className="min-h-screen bg-[#0B0D14] text-gray-100 font-sans p-4 sm:p-8 space-y-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-800 pb-5">
          <div>
            ...
            <p className="text-xs text-gray-400 max-w-3xl mt-1">
              Empirically evaluate candidate virtual try-on models across 10 clothing categories based on Accuracy, Speed (&lt;15s), and Unit Cost (&lt;₹4.00/gen).
            </p>
          </div>
        ...
        {/* Hard Constraints Status Bar */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 font-mono text-xs">
          <div className="p-3.5 rounded-lg bg-[#12151E] border border-gray-800 flex items-center gap-3">
          ...
          <div className="p-3.5 rounded-lg bg-[#12151E] border border-gray-800 flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] text-gray-400 uppercase">Hard Constraint 2: Cost</span>
        ...
        <ProviderStatusCard key={`providers-${refreshKey}`} />
        ...
        <BenchmarkMatrix key={`matrix-${refreshKey}`} />

        {/* Section 3: IDM-VTON Saree & Kurti Optimization Deep Dive */}
        <OptimizationStudy key={`opt-${refreshKey}`} />

        {/* Section 4: Standardized Dataset Manifest */}
        <DatasetManifestViewer />

        {/* Footer info */}
        <div className="text-center text-xs text-gray-500 py-6 border-t border-gray-800 font-mono">
          Vizzle AI Virtual Try-On Evaluation System · Zero Synthesized Benchmark Numbers · Strict Empirical Logging
        </div>
      </div>
    </div>
  );
